...
//        }
    ...
    public int H1(Space inSpace){
        ...
        if(y-1 >= 0 && layout[x][y-1].getSpaceType()=="W"){
            total += ((Wall)layout[x][y-1]).getWallNum();
        }

        if(y+1 < layout.length && layout[x][y+1].getSpaceType()=="W"){
             total += ((Wall)layout[x][y+1]).getWallNum();
         }
    ...
    public int H2(Space inSpace){
        ...
        while(x-counter >= 0 && layout[x-counter][y].getSpaceType()!="W"){
            total ++;
            counter++;
        }
        counter = 1;

        while(x+counter < layout.length && layout[x+counter][y].getSpaceType()!="W"){
             total ++;
             counter++;
        }
         counter = 1;

        while(y-counter >= 0 && layout[x][y-counter].getSpaceType()!="W"){
            total ++;
            counter++;
        }
        counter = 1;
        while(y+counter < layout.length && layout[x][y+counter].getSpaceType()!="W"){
             total ++;
             counter++;
         }
