...
public class BoardObj{
                ...
                for (int i = -1; i <= 1; i++) {
                    for (int j = -1; j <= 1; j++) {
                        if ((i==0 || j==0) && ((currX+i) >= 0) && ((currY+j) >= 0)
                                && ((currX+i) < this.board.length) && ((currY+j) < this.board[0].length)){
    ...
    public boolean isRowValid(int rowNum, int colNum){
        ...
        counter = colNum+1;
        while (valid && noWall && counter < this.board[rowNum].length){
            if (this.getPosition(rowNum,counter) == 'b'){
                valid = false;
            }else {
                if (Character.isDigit(this.getPosition(rowNum, counter))){
    ...
    public boolean isColValid(int rowNum, int colNum){
        ...
        counter = rowNum+1;
        while (valid && noWall && counter < this.board.length){
            if (this.getPosition(rowNum,counter) == 'b'){
                valid = false;
            }else {
